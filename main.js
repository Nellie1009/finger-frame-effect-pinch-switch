...
let effect = "vangogh";
let effectIndex = EFFECTS.findIndex((e) => e.id === effect);
...
// Pinch both hands closed, then open them back into the frame gesture to
// advance filters. The two-frame gates absorb single-frame landmark jitter.
const PINCH_CLOSE_RATIO = 0.5;
const PINCH_OPEN_RATIO = 0.72;
const PINCH_STABLE_FRAMES = 2;
const PINCH_SWITCH_COOLDOWN = 18;
...
function nextEffect() {
  ...
  setEffect(EFFECTS[effectIndex].id);
  toolbar.classList.remove("switched");
  // Restart the CSS animation even when switches happen close together.
  void toolbar.offsetWidth;
  toolbar.classList.add("switched");
}
...
function pinchRatios(hands) {
  return hands.map((lm) => {
    const thumb = toPixel(lm[THUMB_TIP]);
    const index = toPixel(lm[INDEX_TIP]);
    const wrist = toPixel(lm[WRIST]);
    const middle = toPixel(lm[MIDDLE_MCP]);
    const scale = dist(wrist, middle) + 1;
    return dist(thumb, index) / scale;
  });
}

function updatePinchFilterSwitch(hands, targetQuad) {
  ...
  const ratios = pinchRatios(hands);
  const closed = ratios.every((r) => r < PINCH_CLOSE_RATIO);
  const open = ratios.every((r) => r > PINCH_OPEN_RATIO);

  pinchClosedFrames = closed ? pinchClosedFrames + 1 : 0;
  pinchOpenFrames = open && targetQuad ? pinchOpenFrames + 1 : 0;

  if (!pinchPrimed && pinchClosedFrames >= PINCH_STABLE_FRAMES) {
    pinchPrimed = true;
    pinchOpenFrames = 0;
  }
  ...
  if (
    pinchPrimed &&
    pinchCooldown === 0 &&
    pinchOpenFrames >= PINCH_STABLE_FRAMES
  ) {
    nextEffect();
    pinchPrimed = false;
    pinchClosedFrames = 0;
    pinchOpenFrames = 0;
    pinchCooldown = PINCH_SWITCH_COOLDOWN;
  }
}
...
function loop() {
  ...
  updatePinchFilterSwitch(hands, targetQuad);
...
function fakeHand(indexTip, thumbTip, indexMcp, wrist = indexMcp, middleMcp = indexMcp) {
...
function demoPinchAmount(t) {
  const phase = (t % 5) / 5;
  if (phase < 0.2) return 0;
  if (phase < 0.36) return (phase - 0.2) / 0.16;
  if (phase < 0.56) return 1;
  if (phase < 0.76) return 1 - (phase - 0.56) / 0.2;
  return 0;
}
...
function fakeHands(t) {
  ...
  const close = demoPinchAmount(t) * 0.96;
  const leftIndex = { x: 0.74 + ox, y: 0.26 + oy };
  const leftThumbOpen = { x: 0.8 + ox, y: 0.56 + oy };
  const rightIndex = { x: 0.26 - ox, y: 0.28 - oy };
  const rightThumbOpen = { x: 0.2 - ox, y: 0.58 - oy };
  return [
    fakeHand(
      leftIndex,
      lerpPt(leftThumbOpen, leftIndex, close),
      { x: 0.75 + ox, y: 0.4 + oy },
      { x: 0.76 + ox, y: 0.66 + oy },
      { x: 0.75 + ox, y: 0.42 + oy }
    ),
    fakeHand(
      rightIndex,
      lerpPt(rightThumbOpen, rightIndex, close),
      { x: 0.25 - ox, y: 0.44 - oy },
      { x: 0.24 - ox, y: 0.68 - oy },
      { x: 0.25 - ox, y: 0.46 - oy }
    ),
